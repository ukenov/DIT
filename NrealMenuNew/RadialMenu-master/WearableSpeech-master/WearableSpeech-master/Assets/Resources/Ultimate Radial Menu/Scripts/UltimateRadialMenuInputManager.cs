...
public class UltimateRadialMenuInputManager : MonoBehaviour
{
	...
	public virtual void MouseAndKeyboardInput ( ref bool enableMenu, ref bool disableMenu, ref Vector2 input, ref float distance, ref bool inputDown, ref bool inputUp, int radialMenuIndex )
	{
		...
		if( enableButtonKeyboard != string.Empty && !UltimateRadialMenuInformations[ radialMenuIndex ].radialMenu.IsWorldSpaceRadialMenu )
		{
	...
	public virtual void ControllerInput ( ref bool enableMenu, ref bool disableMenu, ref Vector2 input, ref float distance, ref bool inputDown, ref bool inputUp, int radialMenuIndex )
	{
		...
		Vector2 modInput = new Vector2( invertHorizontal ? -Input.GetAxis( horizontalAxisController ) : Input.GetAxis( horizontalAxisController ), invertVertical ? -Input.GetAxis( verticalAxisController ) : Input.GetAxis( verticalAxisController ) );
		...
		// If the user has a enable key assigned...
		if( enableButtonController != string.Empty && !UltimateRadialMenuInformations[ radialMenuIndex ].radialMenu.IsWorldSpaceRadialMenu )
		{
			// Check for the Enable and Disable button keys and set the enable or disable booleans accordingly.
	...
	public virtual void TouchInput ( ref bool enableMenu, ref bool disableMenu, ref Vector2 input, ref float distance, ref bool inputDown, ref bool inputUp, int radialMenuIndex )
	{
		...
		if( Input.touchCount > 0 )
		{
			...
			for( int i = 0; i < Input.touchCount; i++ )
			{
				...
				float menuRadius = ( UltimateRadialMenuInformations[ radialMenuIndex ].radialMenu.BaseTransform.sizeDelta.x / 2 ) * UltimateRadialMenuInformations[ radialMenuIndex ].radialMenu.ParentCanvas.GetComponent<RectTransform>().localScale.x;

				// Store the touch position.
				Vector2 tPosition = Input.GetTouch( i ).position;
				...
				Vector2 modInput = ( tPosition - ( Vector2 )UltimateRadialMenuInformations[ radialMenuIndex ].radialMenu.BasePosition ) / menuRadius;
				
				// Configure the distance of the mouse position from the Radial Menu's base position.
				float dist = Vector2.Distance( UltimateRadialMenuInformations[ radialMenuIndex ].radialMenu.BasePosition, tPosition );

				// If the radial menu is used in world space, then raycast the input.
				if( UltimateRadialMenuInformations[ radialMenuIndex ].radialMenu.IsWorldSpaceRadialMenu )
					RaycastWorldSpaceRadialMenu( ref modInput, ref dist, tPosition, radialMenuIndex );
				...
				if( Input.GetTouch( i ).phase == TouchPhase.Began )
				{
					...
					if( UltimateRadialMenuInformations[ radialMenuIndex ].radialMenu.RadialMenuActive )
					{
						...
						else if( ( dist > menuRadius * deactivationRadius || dist < TouchHoldInformations[ radialMenuIndex ].radialMenu.CalculatedMinRange ) && !UltimateRadialMenuInformations[ radialMenuIndex ].radialMenu.IsWorldSpaceRadialMenu && activationRadius > 0 )
					...
					if( dist < menuRadius * activationRadius )
					{
						...
						if( TouchHoldInformations[ radialMenuIndex ].currentHoldTime >= activationHoldTime )
						{
							...
							if( dynamicPositioning && !UltimateRadialMenuInformations[ radialMenuIndex ].radialMenu.IsWorldSpaceRadialMenu )
								UltimateRadialMenuInformations[ radialMenuIndex ].radialMenu.SetPosition( tPosition );
						}
					}
				}
				else
				...
				if( Input.GetTouch( i ).phase == TouchPhase.Ended )
				{
					...
					if( UltimateRadialMenuInformations[ radialMenuIndex ].radialMenu.CurrentButtonIndex < 0 && dist > menuRadius * activationRadius && !UltimateRadialMenuInformations[ radialMenuIndex ].radialMenu.IsWorldSpaceRadialMenu && activationRadius > 0 )
						UltimateRadialMenuInformations[ radialMenuIndex ].radialMenu.DisableRadialMenu();
				}
			}
		}
