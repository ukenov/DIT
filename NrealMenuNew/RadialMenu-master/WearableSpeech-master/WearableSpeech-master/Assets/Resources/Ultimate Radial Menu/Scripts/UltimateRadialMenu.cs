...
public class UltimateRadialMenu : MonoBehaviour
{
	...
	public class UltimateRadialButton
	{
		...
		public void OnExit ()
		{
			...
			// If the application is playing and the current selected index is the same as this button, then call OnSelect.
			if( Application.isPlaying && radialMenu.CurrentSelectedButtonIndex >= 0 && radialMenu.CurrentSelectedButtonIndex == buttonIndex )
				OnSelect();
			else
			{
	...
	void Update ()
	{
		// If the application is not playing (in edit mode of Unity)...
		if( !Application.isPlaying )
			UpdatePositioning();
	}
	#endif
	...
	void ResetRadialMenu ()
	{
		...
		if( CurrentButtonIndex >= 0 && CurrentButtonIndex < UltimateRadialButtonList.Count )
		{
			if( selectButtonOnInteract && CurrentSelectedButtonIndex == CurrentButtonIndex )
				UltimateRadialButtonList[ CurrentButtonIndex ].OnSelect();
			else
				UltimateRadialButtonList[ CurrentButtonIndex ].OnExit();
		}
		...
		else
		{
			...
			if( useButtonText )
			{
				...
				if( buttonTextOutline )
				{
					...
					Outline textOutline = buttonText.AddComponent<Outline>();
	...
	public void UpdatePositioning ()
	{
		...
		float xPosition = canvasRectTrans.sizeDelta.x * fixedPositioning.x - ( textureSize * fixedPositioning.x );
		float yPosition = canvasRectTrans.sizeDelta.y * fixedPositioning.y - ( textureSize * fixedPositioning.y );
		...
		BaseTransform.localPosition = new Vector3( xPosition, yPosition );
		...
		// Configure the actual size of the transform for calculation.
		float transformActualSize = BaseTransform.sizeDelta.x * canvasRectTrans.localScale.x;

		// Calculate the minimum range.
		CalculatedMinRange = ( transformActualSize / 2 ) * minRange;
		...
		if( infiniteMaxRange )
			...
			CalculatedMaxRange = ( transformActualSize / 2 ) * maxRange;
		...
		if( displayButtonName && nameText != null )
		{
			...
			Vector2 textPosition = Vector2.zero;//( Vector2 )BaseTransform.position - ( BaseTransform.sizeDelta / 2 );
		...
		{
			// Configure text position and size.
			Vector2 textPosition = Vector2.zero;//( Vector2 )BaseTransform.localPosition - ( BaseTransform.sizeDelta / 2 );
		...
		float rotationOffset = centerAngleOffset - ( GetAnglePerButton / 2 );
		...
		for( int i = 0; i < UltimateRadialButtonList.Count; i++ )
		{
			...
			normalPosition.x += ( Mathf.Cos( ( ( angleInRadians * i ) ) + ( 90 * Mathf.Deg2Rad ) + ( centerAngleOffset * Mathf.Deg2Rad ) + ( angleInRadians / 2 ) ) * buttonRadius );
			normalPosition.y += ( Mathf.Sin( ( ( angleInRadians * i ) ) + ( 90 * Mathf.Deg2Rad ) + ( centerAngleOffset * Mathf.Deg2Rad ) + ( angleInRadians / 2 ) ) * buttonRadius );
			...
			if( scaleTransform )
			{
				...
				if( positionModifier != 0 )
				{
					...
					activePosition.x += ( Mathf.Cos( ( ( angleInRadians * i ) ) + ( 90 * Mathf.Deg2Rad ) + ( centerAngleOffset * Mathf.Deg2Rad ) + ( angleInRadians / 2 ) ) * ( buttonRadius + ( buttonRadius * positionModifier ) ) );
					activePosition.y += ( Mathf.Sin( ( ( angleInRadians * i ) ) + ( 90 * Mathf.Deg2Rad ) + ( centerAngleOffset * Mathf.Deg2Rad ) + ( angleInRadians / 2 ) ) * ( buttonRadius + ( buttonRadius * positionModifier ) ) );
				...
				if( pressedPositionModifier != 0 )
				{
					...
					pressedPosition.x += ( Mathf.Cos( ( ( angleInRadians * i ) ) + ( 90 * Mathf.Deg2Rad ) + ( centerAngleOffset * Mathf.Deg2Rad ) + ( angleInRadians / 2 ) ) * ( buttonRadius + ( buttonRadius * selectedPositionModifier ) ) );
					pressedPosition.y += ( Mathf.Sin( ( ( angleInRadians * i ) ) + ( 90 * Mathf.Deg2Rad ) + ( centerAngleOffset * Mathf.Deg2Rad ) + ( angleInRadians / 2 ) ) * ( buttonRadius + ( buttonRadius * selectedPositionModifier ) ) );
				...
				if( selectedPositionModifier != 0 )
				{
					...
					selectedPosition.x += ( Mathf.Cos( ( ( angleInRadians * i ) ) + ( 90 * Mathf.Deg2Rad ) + ( centerAngleOffset * Mathf.Deg2Rad ) + ( angleInRadians / 2 ) ) * ( buttonRadius + ( buttonRadius * selectedPositionModifier ) ) );
					selectedPosition.y += ( Mathf.Sin( ( ( angleInRadians * i ) ) + ( 90 * Mathf.Deg2Rad ) + ( centerAngleOffset * Mathf.Deg2Rad ) + ( angleInRadians / 2 ) ) * ( buttonRadius + ( buttonRadius * selectedPositionModifier ) ) );
				...
				if( disabledPositionModifier != 0 )
				{
					...
					disabledPosition.x += ( Mathf.Cos( ( ( angleInRadians * i ) ) + ( 90 * Mathf.Deg2Rad ) + ( centerAngleOffset * Mathf.Deg2Rad ) + ( angleInRadians / 2 ) ) * ( buttonRadius + ( buttonRadius * disabledPositionModifier ) ) );
					disabledPosition.y += ( Mathf.Sin( ( ( angleInRadians * i ) ) + ( 90 * Mathf.Deg2Rad ) + ( centerAngleOffset * Mathf.Deg2Rad ) + ( angleInRadians / 2 ) ) * ( buttonRadius + ( buttonRadius * disabledPositionModifier ) ) );
			...
			UltimateRadialButtonList[ i ].angle = 90 + centerAngleOffset + ( -angle * i ) - ( angle / 2 );
	...
	public void RemoveAllRadialButtons ( int buttonCount = 0 )
	{
		...
		for( int i = UltimateRadialButtonList.Count - 1; i >= 0; i-- )
		{
			...
			// Else the index is within the minimum range, so just clear the button information.
			else
	...
	public static void SetPosition ( string radialMenuName, Vector3 screenPosition )
	{
		// If there is not a radial menu that has been registered with the targeted radialMenuName, then return. 
		if( !ConfirmUltimateRadialMenu( radialMenuName ) )
			return;
		...
		UltimateRadialMenus[ radialMenuName ].SetPosition( screenPosition );
	}
